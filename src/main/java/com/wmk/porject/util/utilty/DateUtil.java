...
public class DateUtil extends org.apache.commons.lang.time.DateUtils {
	...
	public static final SimpleDateFormat MORE_SHORT_DATE_FORMAT = new SimpleDateFormat("yyyy-M-d");
	public static final SimpleDateFormat SHORT_MONTH_FORMAT = new SimpleDateFormat("yyyy-M");
	public static final SimpleDateFormat LONG_MONTH_FORMAT = new SimpleDateFormat("yyyy-MM");
	...
	public static boolean isSaturdayOrSunday(String dateStr) throws ParseException {
		if (ValidUtil.valid(dateStr)) {
			Date date = SHORT_DATE_FORMAT.parse(dateStr);
			Integer ret = getWeekIndex(date);
			if (ValidUtil.valid(ret) && (ret.equals(1) || ret.equals(7))) {
				return true;
			}
		}
		return false;
	...
	// 将字符串转换成固定格式的字符串
	public static String StringToDateString(String dateStr, SimpleDateFormat format) throws ParseException {
		if (ValidUtil.valid(dateStr) && ValidUtil.valid(format)) {
	...
	// 转换时间格式
	public static String getDateSpecyFormatStr(Date date, SimpleDateFormat format) throws ParseException {
		if (ValidUtil.valid(date) && ValidUtil.valid(format)) {
			return format.format(date);
		}
		return null;
	}
	...
	// 获取当月的第一天的格式
	public static String getFirstDayByMonth(Date date, SimpleDateFormat format) throws ParseException {
		if (ValidUtil.valid(date)) {
			return format.format(getFirstDateOfMonth(date));
		}
		return null;
	}

	// 获取当月的最后一天的格式
	public static String getLastDayByMonth(Date date, SimpleDateFormat format) throws ParseException {
		if (ValidUtil.valid(date)) {
			return format.format(getLastDateOfMonth(date));
		}
		return null;
	}

	// 获取几个月后的同一天
	public static Date getNextMonthDate(Date date, Integer after) throws ParseException {
		if (ValidUtil.valid(date) && ValidUtil.valid(after)) {
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(date);
			calendar.set(Calendar.MONTH, calendar.get(Calendar.MONTH) + after);
	...
	/**
	 * peng 获取日期字符串是星期几的汉字格式
	 * 
	 * @throws ParseException
	 */
	public static String getRealWeekIndexStringByDate(String str) throws ParseException {
		if (ValidUtil.valid(str)) {
			Date date = SHORT_DATE_FORMAT.parse(str);
			Integer index = getWeekIndex(date);
			if (ValidUtil.valid(index)) {
				String week = String.valueOf(index);
	...
	/**
	 * peng 判断一个日期过去，今天还是未来
	 */
	public static Integer judgeTodayFuturePast(String date) {
		Date current = new Date();
		Integer ret = compareDate(parseDate(date, DATE_FORMAT_DATEONLY), current, Calendar.DATE);
		if (ret < 0) {
			return 1;
		} else if (ret == 0) {
			return 2;
	...
	/**
	 * peng 判断一个日期（date1）相对于另一个日期（date2）是过去，当天还是未来
	 */
	public static Integer judgeTodayFuturePast(String date1, String date2) {
		Integer ret = compareDate(parseDate(date1, DATE_FORMAT_DATEONLY), parseDate(date2, DATE_FORMAT_DATEONLY), Calendar.DATE);
		if (ret < 0) {
			return 1;
		} else if (ret == 0) {
			return 2;
	...
	/**
	 * peng 判断一个时间是不是另一个时间范围内
	 * 
	 * @throws ParseException
	 */
	public static boolean judgeIncludeTime(String time1, String time2) throws ParseException {
		Date date1 = HMS_FORMAT.parse(time1);
		Date date2 = HMS_FORMAT.parse(time2);
		Integer ret = compareDate(date1, date2, Calendar.SECOND);
		if (ret <= 0) {
			return true;
		} else {
			return false;
	...
	public static String get24HourTimeStr(String time1) throws ParseException{
		Date date1 = HMS_FORMAT.parse(time1);
		return HMS_FORMAT.format(date1);
	}
	 ...
	 * @throws ParseException
	 */
	public static boolean judgeEquelTime(String time1, String time2) throws ParseException {
		Date date1 = HMS_FORMAT.parse(time1);
		Date date2 = HMS_FORMAT.parse(time2);
		Integer ret = compareDate(date1, date2, Calendar.SECOND);
		if (ret != 0) {
	 ...
	 * @throws ParseException
	 */
	public static String getLaterDay(String str, SimpleDateFormat format) throws ParseException {
		if (ValidUtil.valid(str) && ValidUtil.valid(format)) {
			Date date1 = format.parse(str);
			Date date2 = addDay(date1, 1);
	...
	public static Timestamp toSqlTimestamp(String sDate, String sFmt) {
		...
		// java.sql.Timestamp.value() 要求的格式必须为yyyy-mm-dd
		// hh:mm:ss.fffffffff
	...
	public static Date StringToDate(String dateStr, String reg) {
	...
	public static long getDateDiff(String startDate, String endDate) {
		...
		try {
			...
			diff = (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) > 0 ? (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) : (date2
	...
	public static long getDateDiff(Date date1, Date date2) {
		...
		long diff = (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) > 0 ? (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) : (date2
						.getTime() - date1.getTime()) / (24 * 60 * 60 * 1000);
		return diff;
	}
	 ...
	 * 
	 * @param timeA
	 *            must pased in as HH:mm:ss, HH can be greater than 24
	 * @param timeB
	 *            must pased in as HH:mm:ss, HH can be greater than 24
	 * @return -1 if a < b; 0 if a = b; 1 if a > b
	 */
	public static int CompareTime(String timeA, String timeB) throws Exception {
		String[] timeAComponents = timeA.split(":");
		String[] timeBComponents = timeB.split(":");
		if (timeAComponents.length != 3 || timeBComponents.length != 3) {
			throw new InvalidParameterException("输入时间必须为HH:mm:ss");
		}
		for (int i = 0; i < timeAComponents.length; i++) {
			if (Integer.parseInt(timeAComponents[i]) < Integer.parseInt(timeBComponents[i]))
				return -1;
			else if (Integer.parseInt(timeAComponents[i]) > Integer.parseInt(timeBComponents[i]))
				return 1;
		}
		return 0;
	}

	public static String addHour(String time, Integer hour) throws Exception {
		String[] timeComponents = time.split(":");
		if (timeComponents.length != 3) {
			throw new Exception("输入时间格式错误");
		}
		int resultHour = Integer.parseInt(timeComponents[0]) + hour;
		return (resultHour >= 10 ? resultHour : "0" + resultHour) + time.substring(2);
	}
	 ...
	 * @throws Exception
	 */
	public static String formatCST(String strDate)throws Exception{
		String sdate=null;
		if (strDate != null && "" != strDate) {
