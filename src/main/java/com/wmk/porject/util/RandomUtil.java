...
public final class RandomUtil {
     ...
     * 返回一个定长的随机字符串(只包含大小写字母、数字)
     *
     * @param byteLength 随机字符串长度
     * @return 随机字符串
     */
    public static byte[] HexByte(int byteLength) {
    	byte[] sb = new byte[byteLength];
        Random random = new Random();
        for (int i = 0; i < byteLength; i++) {
        	sb[i] = (byte) ((byte) (0x0f & random.nextInt(16)) << 4 | (byte) (0x0f & random.nextInt(16)));
        }
        return sb;
    }
